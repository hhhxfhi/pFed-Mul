...
import matplotlib  # 新增：用于设置中文字体

# 设置matplotlib，确保中文正常显示
try:
    matplotlib.rcParams["font.family"] = ["SimHei", "WenQuanYi Micro Hei", "Heiti TC"]
    matplotlib.rcParams["axes.unicode_minus"] = False  # 确保负号正确显示
except:
    logging.warning("无法设置中文字体，图表中的中文可能显示为方块")
...
def train_model(GPs, basemodel, super_param, args):
    ...
    for epoch in range(args.epoches):
        ...
        if epoch%args.valid_per_epoch == 0:
            ...
            # 新增：记录每轮指标
            avg_mse = np.mean(mse_eval)
            avg_acc = np.mean(acc_eval)
            epoch_mse.append(avg_mse)
            epoch_acc.append(avg_acc)
            epochs.append(epoch)
    return GPs, basemodel, [weight, theta, noise_reg], epoch_mse, epoch_acc, epochs

def plot_and_save_metrics(epochs, mse_values, acc_values, title, save_path):
    """绘制并保存MSE和Accuracy的变化曲线"""
    plt.figure(figsize=(12, 5))
    
    # 绘制MSE曲线
    plt.subplot(1, 2, 1)
    plt.plot(epochs, mse_values, 'b-', linewidth=2)
    plt.title('平均MSE变化')
    plt.xlabel('轮次')
    plt.ylabel('MSE')
    plt.grid(True)
    ...
    # 绘制Accuracy曲线
    plt.subplot(1, 2, 2)
    plt.plot(epochs, acc_values, 'r-', linewidth=2)
    plt.title('平均准确率变化')
    plt.xlabel('轮次')
    plt.ylabel('准确率')
    plt.grid(True)
    ...
    logging.info(f"指标图表已保存至: {save_path}")
    plt.close()
...
def main():
    ...
    GPs, basemodel, super_params, train_mse, train_acc, epochs = train_model(GPs, basemodel, super_params, args)

    mse_test, acc_test = eval_model(GPs, basemodel, super_params, args, dataset='test')
    avg_mse_final = np.mean(mse_test)
    avg_acc_final = np.mean(acc_test)
    

    logging.info(f"Testing: mse={[round(mse, 4) for mse in mse_test]}, acc={[round(acc, 4) for acc in acc_test]}")
    # 以数据集名称作为文件名保存测试结果
    result_folder = '/autodl-fs/data/task_diversity_BFL-master/result/'
    # 获取当前时间
    ...
    csv_file = os.path.join(result_folder, f'{args.dataset}_{current_time}_epoch_metrics.csv')
    df.to_csv(csv_file, index=False)
    
    # 保存最终结果到TXT文件
    txt_file = os.path.join(result_folder, f'{args.dataset}_{current_time}_final_results.txt')
    with open(txt_file, 'w') as f:
        f.write(f"Final Testing: avg_mse={round(avg_mse_final, 4)}, avg_acc={round(avg_acc_final, 4)}\n")
        f.write(f"Epoch-wise metrics saved to: {csv_file}")
        
    # 绘制并保存训练过程图表
    plot_title = f"{args.dataset}数据集训练过程"
    plot_file = os.path.join(result_folder, f'{args.dataset}_{current_time}_training_metrics.png')
    plot_and_save_metrics(epochs, train_mse, train_acc, plot_title, plot_file)
