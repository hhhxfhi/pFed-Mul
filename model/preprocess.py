...
def load_celeba():
    ...
    partition = pd.read_csv("/autodl-fs/data/task_diversity_BFL-master/data/celeba/Anno/list_eval_partition.csv", sep=' ', header=None, names=['name', 'partition'])
    train = partition[partition['partition'] == 0]
    valid = partition[partition['partition'] == 1]
    test = partition[partition['partition'] == 2]
    reg = pd.read_csv("/autodl-fs/data/task_diversity_BFL-master/data/celeba/Anno/list_landmarks_align_celeba.csv", index_col=0)
    cla = pd.read_csv("/autodl-fs/data/task_diversity_BFL-master/data/celeba/Anno/list_attr_celeba.csv", index_col=0)
...
def load_dogcat():
    def read_traintest_data(datatype, transform):
        path_dog = os.path.join("/autodl-fs/data/task_diversity_BFL-master/data/dogcat", datatype, "dogs")
        path_cat = os.path.join("/autodl-fs/data/task_diversity_BFL-master/data/dogcat", datatype, "cats")
        images = []
        labels_cla = []
        labels_reg = []
...
class cele_dataset(Dataset):
    def __init__(self, part, y, transform):
        ...
        for imgname in self.part:
            image = Image.open("/autodl-fs/data/task_diversity_BFL-master/data/celeba/image/" + imgname)
